...
class AppException implements Exception {
  const AppException(this.code, this.message, {this.error, this.buttonText});
...
class NoInternetError extends AppException {
  const NoInternetError() : super(503, 'No internet connection');
}

class TimeoutError extends AppException {
  const TimeoutError() : super(504, 'The request timed out');
}

class HandshakeError extends AppException {
  const HandshakeError() : super(526, 'SSL handshake failed');
}

class SessionExpiry extends AppException {
  const SessionExpiry({String? message})
    : super(401, message ?? 'Session has expired');
}

class UnknownError extends AppException {
  const UnknownError({String? message})
    : super(520, message ?? 'An unknown error occurred');
}

class ParsingError extends AppException {
  const ParsingError() : super(520, 'Error parsing response data');
}
...
class AppRestrictionError extends AppException {
  AppRestrictionError({int? statusCode, String? message, ErrorModel? error})
    : super(
        statusCode ?? 499,
        message ?? 'App restricted. Please try again after some time.',
        error: error,
      );
}
...
class BadGatewayError extends AppException {
  BadGatewayError({int? statusCode, String? message})
    : super(statusCode ?? 502, message ?? 'Bad gateway');
}

class ServiceUnavailableError extends AppException {
  ServiceUnavailableError({int? statusCode, String? message})
    : super(statusCode ?? 503, message ?? 'Service temporarily unavailable');
}

class GatewayTimeoutError extends AppException {
  GatewayTimeoutError({int? statusCode, String? message})
    : super(statusCode ?? 504, message ?? 'Gateway timeout');
}

class HttpVersionNotSupportedError extends AppException {
  HttpVersionNotSupportedError({int? statusCode, String? message})
    : super(statusCode ?? 505, message ?? 'HTTP version not supported');
}

class ValidationError extends AppException {
  ValidationError({int? statusCode, String? message, ErrorModel? error})
    : super(statusCode ?? 0, message ?? 'Something went wrong', error: error);
}

class DataTransmissionError extends AppException {
  DataTransmissionError({int? statusCode, String? message})
    : super(
        statusCode ?? 602,
        message ??
